...
static const char *TAG = "rover_demo_cli";
...
static void print_settings(const rover_demo_settings_t *s)
{
...
static int save_and_report(const rover_demo_settings_t *s)
{
    esp_err_t err = rover_demo_settings_save(s);
    printf("save: %s\n", esp_err_to_name(err));
    if (err == ESP_OK) {
...
static int cmd_rover_config(int argc, char **argv)
{
    ...
    rover_demo_settings_t s = {0};
    esp_err_t err = rover_demo_settings_load(&s);
    if (err != ESP_OK) {
        printf("load: %s\n", esp_err_to_name(err));
        return 1;
    }
...
esp_err_t rover_demo_cli_init(void)
{
    ...
    return esp_console_cmd_register(&config_cmd);
}

esp_err_t rover_demo_cli_start(void)
{
    if (!s_repl) {
